...
**Correction (2026-09-12, discovered during a final pre-public multi-agent review):** Non-blocker 1's claim above — "this repo just had a full pass removing personal emails from every tracked file and from commit history itself" — turned out to be incomplete in two ways not caught at the time: (1) the `git-filter-repo` rewrite only touched `main`/`dev`; ~32 other merged/closed branches on the remote still carried the pre-rewrite history with real emails until they were found and deleted in the follow-up review; (2) GitHub independently retains a `refs/pull/N/head` ref for every historical PR (105 of them), which is unaffected by any local history rewrite, force-push, or branch deletion — several of these were confirmed to still serve real emails and one real production IP address on request. The stale branches have since been deleted; the `refs/pull/*` exposure was reviewed and consciously accepted as residual risk rather than remediated (the two real fixes — a GitHub Support request to purge cached PR refs, or moving to a fresh repository with no PR history — were both judged disproportionate to the actual risk). Anyone repeating a "scrub git history" exercise on a repo with any PR history should check for this class of gap explicitly; branch-level and even repo-history-level rewrites do not cover it.
