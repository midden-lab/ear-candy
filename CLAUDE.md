...
5. **SQLite schema now has a real (if minimal) migrations pattern.** `migrate.ts` runs `CREATE TABLE IF NOT EXISTS` for the base schema, then a series of guarded `ALTER TABLE ... ADD COLUMN` blocks (checked via `PRAGMA table_info`) for columns added after the initial schema — e.g. `episodes.cover_art_thumb_path`, `settings.favicon_path`, `settings.browser_tab_title`. This pattern is idempotent and safe to run on every startup, including against a production DB with live data. There's still no rollback/backup tooling around this (tracked as a known gap) — migrations must stay additive/nullable-column-only until that exists.
