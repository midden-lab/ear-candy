...
describe('matchesEpisodeQuery', () => {
  ...
  it('matches on a guest name within a comma-separated list, case-insensitively', () => {
    const ep = makeEpisode({ title: 'Season Premiere', guests: 'Sam, Dr. Priya Nair' })
    expect(matchesEpisodeQuery(ep, 'priya')).toBe(true)
    expect(matchesEpisodeQuery(ep, 'SAM')).toBe(true)
  })

  it('does not match unrelated text', () => {
    const ep = makeEpisode({ title: 'Consent 101', guests: 'Sam' })
...
describe('searchAllEpisodes', () => {
  const episodes = [
    makeEpisode({ id: 1, season_id: 1, title: 'Welcome to the Show', guests: 'Sam' }),
    makeEpisode({ id: 2, season_id: 1, title: 'Boundaries and Consent', guests: 'Sam' }),
    makeEpisode({ id: 3, season_id: 2, title: 'Season Premiere', guests: 'Dr. Priya Nair' }),
    makeEpisode({ id: 4, season_id: 9, title: 'The Orgasm Gap, Explained', guests: 'Sam' }),
  ]

  it('flattens matches across every season and tags each with its origin season id', () => {
    const results = searchAllEpisodes(episodes, 'sam')
