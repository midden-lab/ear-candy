...
export default function AudioPlayer() {
  ...
  // Feed the player's actual rendered height into a shared CSS var so
  // AppShell's <main> can reserve exactly enough padding-bottom, on any
  // breakpoint, instead of a hardcoded guess. Zero when nothing should
  // reserve space (no episode, or the full-screen overlay is covering
  // everything anyway).
  useLayoutEffect(() => {
    ...
    if (!el) return
    const update = () => document.documentElement.style.setProperty('--player-h', `${el.offsetHeight}px`)
    update()
      ...
      >
        ...
        >
          ...
          <span
            ...
            onKeyDown={e => {
              if (e.key === 'Enter' || e.key === ' ') { e.stopPropagation(); e.preventDefault(); setPlaying(!playing) }
            }}
